...
export class CompradorListadoCardComponent implements OnInit {
  displayedColumns: string[] = ['RazonSocial', 'RENSPA', 'CUITPersona', 'NombreEstablecimiento', 'NombreLocalidad', 'NombreProvincia'];
  todosLosDatosTabla = [];
  datosTabla = [];
  isLoading = true;
  ...
  onRenspaClick(event) {
    ...
    this.openSnackBar("Comprador con RENSPA " + event.target.innerHTML + " fue seleccionado con exito", "");
  }
