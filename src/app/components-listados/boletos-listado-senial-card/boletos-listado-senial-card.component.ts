...
@Component({
  selector: 'app-boletos-listado-senial-card',
  templateUrl: './boletos-listado-senial-card.component.html',
  styleUrls: ['./boletos-listado-senial-card.component.scss']
})
export class BoletosListadoSenialCardComponent implements OnInit {
  displayedColumns: string[] = [
    'VencimientoBoleto', 'RENSPA', 'RazonSocial', 
    'BoletoSenial', 'BoletoSenialInc', 'BoletoSenialFolio', 
    'Telefono', 'Email'
  ];
  todosLosDatosTabla = [];
  datosTabla = [];
  ...
  productor_date_compare(a, b) {
    ...
    if (a.VencimientoBoletoSenial) {
      let parts = a.VencimientoBoletoSenial.trim().split('-');
    ...
    if (b.VencimientoBoletoSenial) {
      let parts_b = b.VencimientoBoletoSenial.trim().split('-');
